"""
run_v3_ablation_3rel.py — TransE ablation on Fashion v3, 3-relation family.

Trains TransE on each variant listed in ablation_manifest_3rel.json,
using the fixed hyperparameters selected through HPO for the 3-relation
...
Usage:
    python run_v3_ablation_3rel.py
    python run_v3_ablation_3rel.py --variant loo_no_belongs_to
"""
...
ABLATION_DIR = BASE / "ablation_study" / "fashionv3" / "ablation_3rel"
MANIFEST_PATH = ABLATION_DIR / "ablation_manifest_3rel.json"
...
OUTPUT_ROOT = BASE / "ablation_study" / "fashionv3" / "results_3rel"
...
# Fixed best parameters from TransE HPO on the Fashion v3 3-relation family
BEST_PARAMS = {
    "embedding_dim": 128,
...
def setup_root_logger() -> logging.Logger:
    logger = logging.getLogger("ablation_v3_3rel")
...
def setup_variant_logger(
    ...
    logger = logging.getLogger(
        f"ablation_v3_3rel.{variant_name}"
    )
    logger.setLevel(logging.INFO)
