...
Features:
- hidden_size fixed at 64 (from the best TransE v1 HPO)
...
- Resume mechanism: rerun after interruption and continue from the last completed trial
...
OUT_FOLDER  = os.path.join(BASE, "results", "sasrec_hpo")
RESULTS_CSV = os.path.join(OUT_FOLDER, "hpo_results_sasrec_fashion.csv")
...
# ── Custom trainer with per-epoch validation and test evaluation ──────────────────
class EpochTracker(Trainer):
    ...
    - log validation and test metrics for each epoch to a dedicated file
    """
    ...
    def fit(self, train_data, valid_data, test_data,
        ...
        """
        Override of the standard fit method: evaluate on validation and test at every epoch.
...
def run_trial(trial_idx, params, base_path, data_path):
    ...
    try:
        ...
        try:
            ...
            trial_logger.warning(f"  [INDEPENDENT VERIFICATION] Failed: {e}")
        ...
        return {
            ...
            'test_from_best_valid_ndcg':   results.get('test_from_best_valid_ndcg', 'N/A'),
            'test_from_best_valid_recall': results.get('test_from_best_valid_recall', 'N/A'),
            'status':              'OK',
        }
...
if __name__ == '__main__':
    ...
    main_logger.info(f"  hidden_size:   64 (fixed from TransE v1 HPO)")
    ...
    if os.path.exists(RESULTS_CSV):
        main_logger.info(f"  CSV found: resuming from the last completed trial.")
        try:
            ...
            for r in all_results:
                ...
                param_tuple = tuple(str(r.get(k, '')) for k in [
                    'n_layers', 'n_heads', 'learning_rate',
                    'hidden_dropout_prob', 'attn_dropout_prob',
                    'weight_decay', 'loss_type', 'neg_candidate_num'
                ])
            ...
            main_logger.error(f"  CSV read error: {e}. Starting from scratch.")
            all_results       = []
            tested_params_set = set()
            completed_trials  = set()
    ...
    while len(completed_trials) < NUM_TRIALS:
        ...
        # sample parameters
        params = {k: random.choice(list(v))
                  for k, v in PARAM_SPACE.items()}

        # hidden_size=64 must be divisible by n_heads
        if 64 % params['n_heads'] != 0:
            continue
        ...
        param_tuple = (
            ...
            str(params['hidden_dropout_prob']),
            str(params['attn_dropout_prob']),
            str(params['weight_decay']),
            str(params['loss_type']),
            str(params['train_neg_sample_args'].get('candidate_num', 'N/A')),
        )
        ...
        result = run_trial(trial_idx, params, OUT_FOLDER, DATA_PATH)
    ...
    final_df['best_valid_ndcg'] = pd.to_numeric(
        final_df['best_valid_ndcg'], errors='coerce'
    )
    final_df = final_df.sort_values('best_valid_ndcg', ascending=False)

    if len(final_df) > 0:
        best = final_df.iloc[0]
        main_logger.info("\n--- BEST PARAMETERS (by validation NDCG@20) ---")
        for col in ['trial', 'n_layers', 'n_heads', 'learning_rate',
            'hidden_dropout_prob', 'attn_dropout_prob',
            'weight_decay', 'loss_type', 'neg_candidate_num',
            'best_valid_ndcg', 'best_valid_recall',
            'best_valid_epoch',
            'best_test_ndcg', 'best_test_recall', 'best_test_epoch',
            'test_from_best_valid_ndcg',   # additional verification metric
            'test_from_best_valid_recall']: 
            main_logger.info(f"  {col}: {best.get(col, 'N/A')}")

        main_logger.info("\n--- TOP 5 ---")
        main_logger.info(
            final_df.head(5)[[
                'trial', 'n_layers', 'n_heads', 'learning_rate',
                'best_valid_ndcg', 'best_test_ndcg'
            ]].to_string()
        )
    else:
        main_logger.info("No trial completed successfully.")