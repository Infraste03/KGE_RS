"""
Fashion KGE — HPO: TransE WITHOUT belongs_to_brand.

Identical to run_hpo_transe_fashion.py, but excludes the
belongs_to_brand relation from training. Uses only:
  - compatible_with (item -> item)
  - belongs_to      (item -> category)

kg_train.tsv is NOT modified — triples are filtered in memory.
Output is stored in a separate directory: results/hpo_transe_no_brand/
Separate Optuna study: 'transe_fashion_no_brand'
"""
...
# ── SEPARATE output — does not overwrite the original hpo_transe/ ────────────
RESULTS_DIR = os.path.join(BASE, "results", "hpo_transe_no_brand")
...
# ── Data loading — WITH belongs_to_brand FILTER ───────────────────────────────
def load_triples_factories():
    ...
    train_df = train_df[~train_df['relation'].isin(EXCLUDE_RELATIONS)]
    n_after  = len(train_df)
    logger.info(f"  Train: {n_before:,} -> {n_after:,} triples "
                f"(removed {n_before - n_after:,})")
    logger.info(f"  Valid: {len(valid_df):,} triples (unchanged)")
    logger.info(f"  Test:  {len(test_df):,} triples (unchanged)")
...
def main():
    ...
    logger.info("=" * 70)
    logger.info("FASHION — HPO TransE — ABLATION: NO BRAND")
    logger.info("=" * 70)
    ...
    study = optuna.create_study(
        study_name='transe_fashion_no_brand',   # separate study
        storage=storage_url,
        direction='maximize',
    ...
    logger.info("\n" + "=" * 70)
    logger.info("FINAL RESULTS — TransE NO BRAND (TEST set, avg head+tail)")
    logger.info("=" * 70)
    ...
    # ── Direct comparison with the base version (with brand) ──────────────────
    logger.info("\n" + "=" * 70)
    logger.info("COMPARISON WITH BASE VERSION (with brand)")
    logger.info("=" * 70)
    ...
    if os.path.exists(base_metrics_path):
        ...
        for mode in ['naive', 'type_constrained']:
            ...
            logger.info(
                f"  {mode:20s} | with brand: MRR={base_mrr:.4f} | "
