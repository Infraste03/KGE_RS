"""
Fashion KGE — HPO: TransE WITHOUT compatible_with (worst-case ablation).

Analogous to "loo_no_compatible_with" in B2B: the target relation
compatible_with is excluded from TRAINING (the model never sees those
triples during optimization), while compatible_with recovery is still
evaluated on the test set.

The compatible_with relation embedding remains at its random initialization
throughout training because no gradient updates it.

Objective: verify whether the graph structure learned through
belongs_to_brand + belongs_to_category is sufficient to recover
compatible_with, or whether performance drops substantially, as expected
by analogy with B2B. This would confirm that compatible_with is also
structurally necessary in Fashion.

Training uses only:
  - belongs_to_brand    (item -> brand)
  - belongs_to_category (item -> category)

The entity/relation vocabulary is FIXED and derived from the COMPLETE graph
(full_factory). This is particularly important here: without a fixed
vocabulary, 'compatible_with' would disappear entirely from relation_to_id
and evaluate_custom() would raise a KeyError.

kg_train.tsv is NOT modified — triples are filtered in memory.
Output is stored separately in: results/hpo_transe_no_compatible/
Separate Optuna study: 'transe_fashion_no_compatible'
"""
...
# ── SEPARATE output ───────────────────────────────────────────────────────────
RESULTS_DIR = os.path.join(BASE, "results", "hpo_transe_no_compatible")
os.makedirs(RESULTS_DIR, exist_ok=True)

# ── Relation excluded from TRAINING (kept in vocabulary and test) ────────────
EXCLUDE_RELATIONS = ['compatible_with']
...
def load_triples_factories():
    logger.info(f"Loading triples — excluding from training: {EXCLUDE_RELATIONS}")
    ...
    # Essential here: compatible_with must remain in relation_to_id even
    # if it no longer appears in any training triple.
    ...
    if 'compatible_with' not in full_factory.relation_to_id:
        raise ValueError(
            "'compatible_with' not found in the complete vocabulary — check KG_PATH"
        )
    ...
    logger.info(f"  Train:               {train_factory.num_triples:,} triples "
                f"(0 expected for compatible_with)")
    logger.info(f"  Valid:               {valid_factory.num_triples:,} triples")
    logger.info(f"  Test:                {test_factory.num_triples:,} triples")

    # Explicit sanity check: compatible_with must be absent from training
    # but still present in relation_to_id.
    rel_id = train_factory.relation_to_id['compatible_with']
    n_cw_train = (train_factory.mapped_triples[:, 1] == rel_id).sum().item()
    n_cw_test  = (test_factory.mapped_triples[:, 1] == rel_id).sum().item()
    logger.info(f"  Sanity check — compatible_with: train={n_cw_train} "
                f"(expected 0), test={n_cw_test} (expected > 0)")
    if n_cw_train != 0:
        raise RuntimeError("compatible_with found in training: filtering failed")
    if n_cw_test == 0:
        raise RuntimeError("compatible_with absent from test: evaluation impossible")
...
def evaluate_custom(model, test_factory, train_factory,
    ...
    # The known-target index is effectively built from validation and test
    # for compatible_with, because training no longer contains this relation.
...
def main():
    ...
    logger.info("=" * 70)
    logger.info("FASHION — HPO TransE — ABLATION: NO COMPATIBLE_WITH (worst-case)")
    logger.info("=" * 70)
    logger.info(f"  Relations excluded from training: {EXCLUDE_RELATIONS}")
    ...
    study = optuna.create_study(
        study_name='transe_fashion_no_compatible',   # separate study
        storage=storage_url,
        direction='maximize',
    ...
    logger.info("\n" + "=" * 70)
    logger.info("FINAL RESULTS — TransE NO COMPATIBLE_WITH (TEST set, avg head+tail)")
    logger.info("=" * 70)
    ...
    if os.path.exists(base_metrics_path):
        ...
        for mode in ['naive', 'type_constrained']:
            ...
            logger.info(
                ...
                f"without compatible_with: MRR={new_mrr:.4f} | "
