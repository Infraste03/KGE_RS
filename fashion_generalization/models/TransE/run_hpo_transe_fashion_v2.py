"""
Fashion KGE — HPO v2: Hyperparameter Optimization for TransE.

Version 2 — differences compared with v1:
  - RESULTS_DIR: results/hpo_transe_v2  (does not overwrite v1)
  - study_name: transe_fashion_v2       (separate Optuna study)
  - TRAINING_EPOCHS_HPO: 150            (previously 100)
  - num_negs_per_pos: [32, 64, 128, 256]    (higher range)

Same data as v1 — no additional preprocessing.
"""
...
RESULTS_DIR = os.path.join(BASE, "results", "hpo_transe_v2")  # v2
...
def suggest_hyperparameters(trial):
    return {
        'embedding_dim': trial.suggest_categorical(
            'embedding_dim', [32, 64, 128, 200, 256, 400]  # 400 removed, 32 added
        ),
        'learning_rate': trial.suggest_float(
            'learning_rate', 1e-4, 1e-2, log=True
        ),
        'num_negs_per_pos': trial.suggest_categorical(
            'num_negs_per_pos', [8, 16, 32, 64, 70,  128, 256]    # higher range
        ),
...
def main():
    ...
    logger.info("=" * 70)
    logger.info("FASHION — HPO TransE v2")
    logger.info("=" * 70)
    ...
    logger.info(f"  Data:           {BASE}/data/processed/ (same as v1)")
    ...
    logger.info("\n" + "=" * 70)
    logger.info("FINAL RESULTS — TransE v2 (TEST set, avg head+tail)")
    logger.info("=" * 70)
