"""
Fashion KGE — HPO: TransE WITHOUT belongs_to_brand (v2 — fixed vocabulary).

Retraining of the no_brand variant with the same correction already
applied to no_category and no_compatible_with: the entity/relation
vocabulary is FIXED and derived from the COMPLETE graph (full_factory),
not from the filtered array.

The old run_hpo_transe_fashion_no_brand.py did not include this correction:
brand entities disappeared from entity_to_id because, after filtering,
they no longer appeared in any triple. This produced a shorter vocabulary
that differed from full/no_category/no_compatible_with.

This breaks alignment with the SharedEmbedding during warm-starting
for alternate learning (Step 4).
...
Output is stored separately from the old run:
results/hpo_transe_no_brand_v2/

Separate Optuna study: 'transe_fashion_no_brand_v2'
"""
...
# ── SEPARATE output — does not affect the old hpo_transe_no_brand/ ────────────
RESULTS_DIR = os.path.join(BASE, "results", "hpo_transe_no_brand_v2")
os.makedirs(RESULTS_DIR, exist_ok=True)

# ── Relation to exclude ───────────────────────────────────────────────────────
EXCLUDE_RELATIONS = ['belongs_to_brand']

# ── HPO configuration — identical to the other variants ──────────────────────
N_TRIALS              = 30
...
def load_triples_factories():
    ...
    if EXCLUDE_RELATIONS[0] not in full_train_df['relation'].unique():
        raise ValueError(
            f"'{EXCLUDE_RELATIONS[0]}' not found in the KG. "
...
# ── Search space — identical to the other variants ────────────────────────────
def suggest_hyperparameters(trial):
    return {
        'embedding_dim': 64,   # FIXED — must match kge_dim in Step 4 for warm-start
...
def main():
    ...
    logger.info("=" * 70)
    logger.info("FASHION — HPO TransE — ABLATION: NO BRAND (v2, fixed vocabulary)")
    logger.info("=" * 70)
    ...
    study = optuna.create_study(
        study_name='transe_fashion_no_brand_v2',   # separate study
        storage=storage_url,
        direction='maximize',
    ...
    logger.info("\n" + "=" * 70)
    logger.info("FINAL RESULTS — TransE NO BRAND v2 (TEST set, avg head+tail)")
    logger.info("=" * 70)
    ...
    if os.path.exists(base_metrics_path):
        ...
        for mode in ['naive', 'type_constrained']:
            ...
            logger.info(
                ...
                f"without brand: MRR={new_mrr:.4f} | "
