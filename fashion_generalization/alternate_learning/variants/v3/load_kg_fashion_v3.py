"""
load_kg_fashion_v3.py — Load the Knowledge Graph for Fashion alternate learning v3.
...
Entity types supported by the Fashion v3 KG:
  - item      : products (item_XXXXX)
  - category  : categories (category_XXXXX)
  - brand     : brands (brand_XXXXX)
  - pricetier : price-tier entities (pricetier_XXXXX)
  - poptier   : popularity-tier entities (poptier_XXXXX)
...
Usage:
    ...
    kg = load_kg_fashion(
        train_path='data/processed_v3/kg_train.tsv',
        valid_path='data/processed_v3/taskA_valid.tsv',
        test_path='data/processed_v3/taskA_test.tsv',
    )
"""
...
# --------------------------------------------------------------------------- #
# Entity type prefixes supported by the Fashion KG
# --------------------------------------------------------------------------- #

ENTITY_TYPE_PREFIXES = ['item', 'category', 'brand', 'pricetier', 'poptier']
...
def _entity_type(entity_string: str) -> str:
    """
    Extract the entity type prefix from an entity string
    (e.g. 'item_1234' -> 'item').
    """
...
def _build_mappings(all_entities: set, all_relations: set):
    ...
    Alphabetical ordering guarantees deterministic mappings:
    the same dataset always produces the same IDs.
    """
...
def _triples_to_ids(
    ...
    if unknown_entities or unknown_relations:
        ...
        if unknown_entities:
            ...
            msg += (
                f"{len(unknown_entities)} unknown entities "
                f"(sample: {sample}). "
            )
...
def load_kg_fashion(
    ...
    Returns:
        dict containing:
          ...
          - 'entity_type_indices' : dict[str, list[int]]
              Keys: 'item', 'category', 'brand', 'pricetier', 'poptier'
    """
    if verbose:
        logger.info("=" * 70)
        logger.info(
            "LOADING FASHION V3 KG FOR STEP 4 "
            "(ALTERNATE LEARNING)"
        )
        logger.info("=" * 70)
    ...
    if verbose:
        logger.info(
            f"Train triples: {len(train_df):,} from {train_path}"
        )
        logger.info(
            f"Valid triples: {len(valid_df):,} from {valid_path}"
        )
        logger.info(
            f"Test  triples: {len(test_df):,} from {test_path}"
        )
    ...
    if verbose:
        ...
        logger.info(
            f"Total entities   : {len(entity_to_id):,}"
        )

        for prefix in ENTITY_TYPE_PREFIXES:
            logger.info(
                f"  {prefix:20s}: "
                f"{len(entity_type_indices[prefix]):,}"
            )

        logger.info(
            f"Total relations  : {len(relation_to_id):,}"
        )

        for rel, rid in sorted(
            relation_to_id.items(),
            key=lambda x: x[1]
        ):
            n_in_train = (
                train_triples[:, 1] == rid
            ...
            logger.info(
                f"  {rel:30s} (id={rid}): "
                f"{n_in_train:,} training triples"
            )

        logger.info(
            f"Train triples    : {len(train_triples):,}"
        )

        logger.info(
            f"Valid triples    : {len(valid_triples):,}"
        )
...
if __name__ == "__main__":
    ...
    assert (
        ...
        "Negative IDs found in train_triples"
    ...
    # Verify that the main entity types are present
    assert len(
    ...
    assert len(
        kg['entity_type_indices']['category']
    ) > 0, \
        "No categories found"

    assert len(
        kg['entity_type_indices']['brand']
    ) > 0, \
        "No brands found"

    # Verify that entity-type sets are disjoint and cover all entities
    total_typed = sum(
        len(v)
        for v in kg['entity_type_indices'].values()
    )

    assert total_typed == kg['num_entities'], \
        (
            f"entity_type_indices do not cover all entities: "
            f"{total_typed} vs {kg['num_entities']}"
        )

    # Verify that compatible_with is present among the relations
    assert 'compatible_with' in kg['relation_to_id'], \
        (
            "Relation 'compatible_with' not found — "
            "check the TSV files"
        )
    ...
    print(
        f"  items                 = "
        f"{len(kg['entity_type_indices']['item']):,}"
    )
    ...
    print(
        f"  train_triples shape   = "
        f"{kg['train_triples'].shape}"
    )

    print(
        f"  valid_triples shape   = "
        f"{kg['valid_triples'].shape}"
    )

    print(
        f"  test_triples shape    = "
        f"{kg['test_triples'].shape}"
    )