...
HPO wrapper for Fashion Step 4 with Cross-Entropy loss using Optuna TPE.

It follows the same general structure as the B2B HPO wrapper, with the
following Fashion-specific differences:
  - Runs run_step4_fashion_ce.py.
  - HPO results are stored under hpo_results_fashion_ce/.
  - Step 4 results are stored under hpc_step4_results/.
  - The first enqueued trial uses Fashion-specific values
    (margin=8.684, dropout=0.5).
  - Log parsing is adapted to the Fashion format:
        ==> Valid: ... | Test: ...
...
Example on HPC:
  python run_hpo_step4_fashion_ce.py \
      --base-config configs/step4_hpc_config_fashion_ce.yaml \
      --trials 30 \
      --epochs 50 \
      --scheduling one_to_one \
      --storage sqlite:///hpo_results_fashion_ce/optuna_fashion.db
...
DEFAULT_BASE_CONFIG   = PROJECT_ROOT / 'configs' / 'step4_hpc_config_fashion_ce.yaml'
DEFAULT_RESULTS_ROOT = PROJECT_ROOT / 'hpo_results_fashion_ce'
DEFAULT_SEEDS  = [2020]
DEFAULT_TRIALS = 30
DEFAULT_EPOCHS = 50
...
def run_trial_process(config_path, epochs, env=None, timeout=None):
    """Run run_step4_fashion_ce.py as a subprocess."""
    cmd = [
        sys.executable,
        str(PROJECT_ROOT / 'run_step4_fashion_ce.py'),
...
def parse_fashion_log(out: str):
    """
    Extract metrics from the run_step4_fashion_ce.py log.
...
def objective_factory(base_cfg, args, study_dir):
    ...
    def objective(trial):
        try:
            ...
            if retcode != 0:
                append_skipped_trial(
                    study_dir=study_dir,
                    trial_number=trial.number,
                    reason=f'run_step4_fashion_ce exitcode={retcode}',
        ...
        # test metrics corresponding to the best validation model
        ...
        best_metrics_path = (
            PROJECT_ROOT
            / 'hpc_step4_results'
            / f"step4_{sched}_ce{run_tag_actual}"
            / 'best_metrics.json'
        )
        ...
        # Copy relevant trial outputs into trial_N/outputs/.
        # Per-epoch checkpoints are not retained.
        src_folder = (
            PROJECT_ROOT
            / 'hpc_step4_results'
            / f"step4_{trial_cfg['training']['scheduling']}_ce"
...
def main():
    parser = argparse.ArgumentParser(
        description='HPO Step 4 Fashion CE (Optuna TPE)'
    )
    ...
    print('Fashion CE HPO completed.')
