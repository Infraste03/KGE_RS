...
Step 4 orchestrator — Fashion Alternate Learning.

What it does:
  1. Reads the YAML configuration, with optional CLI overrides.
...
Usage:
  python run_step4_fashion.py --config configs/step4_config_fashion.yaml
  python run_step4_fashion.py --config configs/step4_config_fashion.yaml --scheduling epoch --epochs 50
  python run_step4_fashion.py --config configs/step4_config_fashion.yaml --seed 42 --run-tag seed42
...
Differences compared with B2B:
  ...
  - best_r20 is initialized to -1.0 so that best_model.pt is always
    saved after the first evaluated epoch.
  - Standalone SASRec baseline:
        Recall@20 = 0.0964
        NDCG@20   = 0.0889

Location:
    fashion_generalization/alternate_learning/run_step4_fashion.py
"""
...
from alternate_loop_fashion import AlternateTrainerFashion
...
_eu._EVAL_TASK_B_DEBUG_PRINTED = True  # skip diagnostics
...
def load_config():
    parser = argparse.ArgumentParser(description="Step 4 Fashion — Alternate Learning")
...
def main():
    ...
    out_dir = os.path.join(THIS_DIR, "hpc_step4_results", f"step4_{sched}{run_tag}")
    logger = setup_logging(out_dir)

    logger.info("=" * 70)
    logger.info("STEP 4 FASHION — ALTERNATE LEARNING TRAINING")
    logger.info("=" * 70)
    ...
    logger.info(f"  Total KG entities: {space.num_total_entities:,}")

    kg = load_kg_fashion(KG_TRAIN, KG_VALID, KG_TEST, verbose=False)
    type_index = build_type_index(kg)
    logger.info(f"  Items: {len(type_index['item']):,}, "
    ...
    recbole_cfg = RecBoleConfig(
        model='SASRec', dataset='fashion', config_dict=recbole_cfg_dict
    )
    ...
    train_data_recbole, valid_data, test_data = data_preparation(
        recbole_cfg, recbole_dataset
    )
    num_items_recbole = recbole_dataset.item_num
    logger.info(f"  item_num (including PAD): {num_items_recbole:,}")
    logger.info(f"  user_num                : {recbole_dataset.user_num:,}")

    # ===================================================================== #
    # STEP 4: r2u tensor (RecBole -> Unified)
    # ===================================================================== #
    logger.info("\n[4/10] Building r2u tensor (RecBole -> Unified)...")
    r2u = torch.full((num_items_recbole,), UNIFIED_PADDING, dtype=torch.long)
    for recbole_id in range(num_items_recbole):
        uid = space.recbole_to_unified(recbole_id)
        if uid is not None:
            r2u[recbole_id] = uid
    n_mapped = (r2u != UNIFIED_PADDING).sum().item()
    logger.info(f"  r2u shape={r2u.shape}, mapped={n_mapped:,}, "
                f"padding entries={num_items_recbole - n_mapped}")
    assert n_mapped == num_items_recbole - 1, \
        f"Expected {num_items_recbole-1} mapped items, found {n_mapped}"
    ...
    logger.info(f"  KG loader: {len(kg_train_loader.dataset):,} triples, "
    ...
    logger.info(f"  Rec loader: {len(sasrec_train_loader.dataset):,} sequences, "
    ...
    model = JointAlternateModelFashion(
        ...
        max_seq_length=recbole_cfg_dict.get('MAX_ITEM_LIST_LENGTH', 50),
    ...
    pykeen_e2id = dict(train_factory.entity_to_id)
    pykeen_r2id = dict(train_factory.relation_to_id)
    ...
    logger.info(f"  Mean L2 norm of SharedEmbedding (first 100 rows): "
                f"{model.shared_embedding.embedding.weight[:100].norm(dim=1).mean():.4f}")
    logger.info(f"  Mean L2 norm of original SASRec item_embedding  : "
    ...
    ckpt_latest_path = os.path.join(out_dir, 'checkpoint_latest.pth')
    start_epoch      = 1
    training_history = []

    if os.path.exists(ckpt_latest_path):
        try:
            logger.info(f"  Checkpoint found: {ckpt_latest_path}")
            ...
            if 'optimizer_state' in ckpt and ckpt['optimizer_state'] is not None:
                try:
                    ...
                    logger.warning("  Optimizer state is incompatible and will be ignored.")
            try:
                _random.setstate(ckpt['random_state'])
                np.random.set_state(ckpt['numpy_state'])
                torch.set_rng_state(ckpt['torch_rng'])
                if device.type == 'cuda' and ckpt.get('cuda_rngs'):
                ...
                logger.warning("  RNG state was not restored.")
            training_history = ckpt.get('training_history', [])
            start_epoch      = ckpt.get('epoch', 0) + 1
            logger.info(f"  Resuming from epoch {start_epoch}")
        except Exception as e:
            logger.warning(f"  Unable to load checkpoint: {e}. Starting from scratch.")
    ...
    reset_eval_debug()
    val0  = evaluate_task_b_fashion(model, valid_data, r2u)
    test0 = evaluate_task_b_fashion(model, test_data,  r2u)
    r20_val0  = val0['Recall@20']
    ndcg_val0 = val0['NDCG@20']
    r20_test0  = test0['Recall@20']
    ndcg_test0 = test0['NDCG@20']
    logger.info(f"  Epoch 0 | Valid: R@20={r20_val0:.4f} NDCG@20={ndcg_val0:.4f} | "
                f"Test: R@20={r20_test0:.4f} NDCG@20={ndcg_test0:.4f}")
    logger.info(f"  (Standalone RecBole baseline: R@20={BASELINE_RECALL20}, "
                f"NDCG@20={BASELINE_NDCG20})")

    # ===================================================================== #
    # STEP 9: Training loop
    # ===================================================================== #
    logger.info(f"\n[9/10] Alternate training — {config['training']['epochs']} epochs...")

    # IMPORTANT:
    # best_r20 is initialized to -1.0 rather than to r20_val0.
    ...
    for epoch in range(start_epoch, num_epochs + 1):
        ...
        logger.info(
            ...
            f"batchA={stats['batches_a']} batchB={stats['batches_b']} | "
        ...
        if epoch % eval_step == 0:
            ...
            # Validation
            reset_eval_debug()
            val_m  = evaluate_task_b_fashion(model, valid_data, r2u)
            ...
            # Test
            reset_eval_debug()
            test_m  = evaluate_task_b_fashion(model, test_data, r2u)
            ...
            logger.info(
                f"  -==> Valid: R@20={r20_v:.4f} NDCG@20={ndcg_v:.4f} | "
                f"Test: R@20={r20_t:.4f} NDCG@20={ndcg_t:.4f}"
            )

            # Save best model according to validation Recall@20
            if r20_v > best_r20_val:
                ...
                logger.info(
                f"  => NEW BEST VALID | epoch={epoch} | "
                f"R@20={r20_v:.4f} NDCG@20={ndcg_v:.4f} | "
                f"corresponding test: R@20={r20_t:.4f} NDCG@20={ndcg_t:.4f}"
            )
            ...
            if r20_t > best_r20_test:
                ...
                torch.save(
                    model.state_dict(),
                    os.path.join(out_dir, "best_model_on_test.pt")
                )
                logger.info(
                f"  => NEW BEST TEST  | epoch={epoch} | "
                f"R@20={r20_t:.4f} NDCG@20={ndcg_t:.4f}"
            )
        ...
        # torch.save(ckpt, os.path.join(out_dir, f'checkpoint_epoch_{epoch}.pth'))
    ...
    if os.path.exists(best_model_path):
        ...
        logger.info(f"  Loaded best_model.pt (best validation epoch {best_epoch_val})")
    else:
        logger.warning("  best_model.pt not found — using last_model.pt")

    reset_eval_debug()
    final_test = evaluate_task_b_fashion(model, test_data, r2u)
    r20_final  = final_test['Recall@20']
    ndcg_final = final_test['NDCG@20']

    logger.info("=" * 70)
    logger.info("FINAL RESULTS — TEST SET (validation-selected model)")
    logger.info("=" * 70)
    logger.info(f"  Recall@20 : {r20_final:.4f}  "
                f"(standalone SASRec baseline: {BASELINE_RECALL20})")
    logger.info(f"  NDCG@20   : {ndcg_final:.4f}  "
                f"(standalone SASRec baseline: {BASELINE_NDCG20})")
    logger.info(f"  Best epoch (validation): {best_epoch_val}")
    ...
    logger.info(f"\nTraining completed. Results saved in {out_dir}")
