...
It follows the same structure as the B2B implementation (run_hpo_step4.py),
with the following Fashion-specific differences:
  - Runs run_step4_fashion.py instead of run_step4.py
  - Results are stored under hpo_results_fashion/
  - Step 4 results are stored under hpc_step4_results/
  - The first enqueued trial uses Fashion-specific values
  - Log parsing is adapted to the Fashion format (==> instead of
    [Validation]/[Test])
  - Fashion best_metrics.json uses the following keys:
...
Example on HPC:
  python run_hpo_step4_fashion.py \
...
Automatic resume:
Optuna resumes from the SQLite database when the same study/storage
configuration is used again after an interrupted job.
"""
...
# Paths relative to alternate_learning/
...
def objective_factory(base_cfg, args, study_dir):
    ...
    def objective(trial):
        try:
            ...
            # Run trial
            retcode, out = run_trial_process(cfg_path, args.epochs, env=env, timeout=args.timeout)
        ...
        best_val_r20, best_val_ndcg, last_test_r20, last_test_ndcg = parse_fashion_log(out)
        ...
        # Read best_metrics.json to recover the selected epochs and
        # the test metrics corresponding to the best validation model
        sched   = trial_cfg['training']['scheduling']
        run_tag_actual = trial_cfg['training'].get('run_tag', '')
        best_metrics_path = PROJECT_ROOT / ('hpc_step4_results') / f"step4_{sched}{run_tag_actual}" / 'best_metrics.json'
        ...
        with open(os.path.join(trial_dir, f'trial_{trial.number}_meta.json'), 'w',
                  encoding='utf-8') as f:
            json.dump(meta, f, indent=2, ensure_ascii=False)

        # Copy trial outputs into trial_N/outputs/.
        # Per-epoch checkpoints are not retained here.
        src_folder = PROJECT_ROOT / ('hpc_step4_results') / f"step4_{trial_cfg['training']['scheduling']}{trial_cfg['training'].get('run_tag', '')}"
        dest_outputs = os.path.join(trial_dir, 'outputs')
        try:
            if src_folder.exists():
                if os.path.exists(dest_outputs):
                    shutil.rmtree(dest_outputs)
                # Copy only the relevant files, not per-epoch checkpoints
                os.makedirs(dest_outputs, exist_ok=True)
                for fname in ['best_model.pt', 'best_model_on_test.pt',
                              'last_model.pt', 'best_metrics.json',
                              'training_history.json', 'run.log']:
                    src_file = src_folder / fname
                    if src_file.exists():
                        shutil.copy(src_file, os.path.join(dest_outputs, fname))
        ...
        with open(trials_csv, 'a', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow([
                trial.number, ts, float(result),
                best_val_r20, best_val_ndcg,
                last_test_r20, last_test_ndcg,
                test_from_best_valid_r20, test_from_best_valid_ndcg,
                seed, lr, batch_size_sasrec, batch_size_kge,
                num_neg_b, margin_a, hidden_dropout, attn_dropout,
        ...
        with open(best_valid_csv, 'a', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow([
                trial.number, best_epoch_valid,
                lr, batch_size_sasrec, batch_size_kge, num_neg_b,
                margin_a, hidden_dropout, attn_dropout,
                best_val_r20, best_val_ndcg,
                test_from_best_valid_r20, test_from_best_valid_ndcg,
            ])
        ...
        with open(best_test_csv, 'a', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            writer.writerow([
                trial.number, best_epoch_test,
                lr, batch_size_sasrec, batch_size_kge, num_neg_b,
                margin_a, hidden_dropout, attn_dropout,
                best_test_r20, best_test_ndcg,
            ])
...
def main():
    parser = argparse.ArgumentParser(description='HPO Step 4 Fashion (Optuna TPE)')
    ...
    parser.add_argument('--scheduling',  type=str,
                        choices=['one_to_one', 'epoch'], default='one_to_one')
    ...
    if args.study_name is None:
        args.study_name = f"step4_fashion_{args.scheduling}_{time.strftime('%Y%m%d_%H%M%S')}"
    ...
    if args.storage:
        ...
        db_path = os.path.abspath(
            os.path.join(study_dir, f"optuna_{args.study_name}.db")
        )
    ...
    # First trial: verified Fashion values
    # DIFFERENT from B2B (margin=5.19, dropout=0.4)
    ...
    if not os.path.exists(trials_csv):
        with open(trials_csv, 'w', newline='', encoding='utf-8') as f:
            csv.writer(f).writerow([
                ...
                'last_test_recall@20', 'last_test_ndcg@20',
                'test_from_best_valid_recall', 'test_from_best_valid_ndcg',
                'seed', 'learning_rate', 'batch_size_sasrec', 'batch_size_kge',
                'num_neg_b', 'margin_loss', 'hidden_dropout', 'attn_dropout',
                'metrics', 'trial_dir',
            ])
    ...
    if not os.path.exists(best_valid_csv):
        with open(best_valid_csv, 'w', newline='', encoding='utf-8') as f:
            csv.writer(f).writerow([
                ...
                'learning_rate', 'batch_size_sasrec', 'batch_size_kge',
                'num_neg_b', 'margin_loss', 'hidden_dropout', 'attn_dropout',
