...
Example on HPC:
  ...
  python ext_epoch_run_hpo_step4_fashion.py \
...
def objective_factory(
    ...
    def objective(trial):
        ...
        try:
            ...
            # The search ranges follow the corresponding Fashion experiment.
            ...
            num_neg_b = trial.suggest_categorical(
                'num_neg_b',
                [1, 2, 4]
            )
            ...
            # Set run_tag before creating the trial configuration.
            # This prevents different HPO trials from sharing the same
            # Step 4 output directory.
            run_tag = (
                f"_hpo_trial{trial.number}"
                f"_seed{seed}"
            )
        ...
        # HPO objective: best validation Recall@20
        result = best_val_r20
        ...
        # Copy the relevant trial outputs into trial_N/outputs/.
        ...
        src_folder = (
            ...
            / (
                f"step4_"
                f"{trial_cfg['training']['scheduling']}"
...
def main():
    ...
    if len(study.trials) == 0:
        ...
        # Trial 25 — best completed R@20 = 0.0990
        study.enqueue_trial({
            'learning_rate': 1.188e-4,
            'batch_size_sasrec': 128,
            'batch_size_kge': 2048,
            'num_neg_b': 1,
            'margin_loss': 4.457,
            'hidden_dropout': 0.314,
            'attn_dropout': 0.375,
        })

        # Trial 18 — interrupted at epoch 32 with R@20=0.1159
        study.enqueue_trial({
            'learning_rate': 1.547e-4,
            'batch_size_sasrec': 128,
            'batch_size_kge': 2048,
            'num_neg_b': 1,
            'margin_loss': 7.150,
            'hidden_dropout': 0.105,
            'attn_dropout': 0.595,
        })

        # Trial 8 — R@20=0.0946, stable configuration
        study.enqueue_trial({
            'learning_rate': 1.886e-4,
            'batch_size_sasrec': 256,
            'batch_size_kge': 2048,
            'num_neg_b': 1,
            'margin_loss': 9.063,
            'hidden_dropout': 0.228,
            'attn_dropout': 0.552,
        })
